...
import { UserContext } from "./selectedCart";

const ConfirmedCard = () => {
  const { cartItems, content, total } = useContext(UserContext);
  return (
              ...
              (objKey) =>
                cartItems[objKey] !== 0 && (
                  <li key={objKey}>
                    <div>
                      <div>
                        <img src={content[objKey].image.thumbnail}></img>
                      </div>
                      <div>
                        {content[objKey].name}
                        <div>
                      ...
                      <p>${content[objKey].price * cartItems[objKey]}</p>
